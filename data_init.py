...
def initialize_data_files():
    ...
    if not os.path.exists('data/sales.csv'):
        sales_df = pd.DataFrame(columns=[
            'Order_ID', 'Date', 'Time', 'Product', 'Quantity', 'Price', 'Total'
        ])
...
def generate_sample_data():
    ...
    inventory_items = [
        {'Name': 'Coffee beans', 'Unit': 'g', 'Quantity': 5000, 'Cost': 500000, 'Date': (datetime.now() - timedelta(days=30)).strftime('%Y-%m-%d')},
        {'Name': 'Milk', 'Unit': 'ml', 'Quantity': 10000, 'Cost': 300000, 'Date': (datetime.now() - timedelta(days=15)).strftime('%Y-%m-%d')},
        {'Name': 'Sugar', 'Unit': 'g', 'Quantity': 3000, 'Cost': 100000, 'Date': (datetime.now() - timedelta(days=20)).strftime('%Y-%m-%d')},
        {'Name': 'Chocolate powder', 'Unit': 'g', 'Quantity': 1000, 'Cost': 150000, 'Date': (datetime.now() - timedelta(days=25)).strftime('%Y-%m-%d')},
        {'Name': 'Tea leaves', 'Unit': 'g', 'Quantity': 2000, 'Cost': 400000, 'Date': (datetime.now() - timedelta(days=10)).strftime('%Y-%m-%d')},
        {'Name': 'Cups', 'Unit': 'pcs', 'Quantity': 500, 'Cost': 500000, 'Date': (datetime.now() - timedelta(days=45)).strftime('%Y-%m-%d')},
        {'Name': 'Straws', 'Unit': 'pcs', 'Quantity': 1000, 'Cost': 50000, 'Date': (datetime.now() - timedelta(days=40)).strftime('%Y-%m-%d')},
    ]
    
    # Sample products with recipes
    products = [
        {'Name': 'Espresso', 'Price': 30000, 'Category': 'Coffee', 'COGS': 15000},
        {'Name': 'Cappuccino', 'Price': 45000, 'Category': 'Coffee', 'COGS': 25000},
        {'Name': 'Latte', 'Price': 50000, 'Category': 'Coffee', 'COGS': 28000},
        {'Name': 'Americano', 'Price': 40000, 'Category': 'Coffee', 'COGS': 20000},
        {'Name': 'Green Tea', 'Price': 35000, 'Category': 'Tea', 'COGS': 18000},
        {'Name': 'Black Tea', 'Price': 35000, 'Category': 'Tea', 'COGS': 17000},
        {'Name': 'Hot Chocolate', 'Price': 45000, 'Category': 'Chocolate', 'COGS': 22000},
    ]
    ...
    if not os.path.exists('data/inventory.csv') or os.path.getsize('data/inventory.csv') <= len("Name,Unit,Quantity,Cost,Date\n"):
        pd.DataFrame(inventory_items).to_csv('data/inventory.csv', index=False)
    
    if not os.path.exists('data/products.csv') or os.path.getsize('data/products.csv') <= len("Name,Price,Category,COGS\n"):
    ...
    if not os.path.exists('data/sales.csv') or os.path.getsize('data/sales.csv') <= len("Order_ID,Date,Time,Product,Quantity,Price,Total\n"):
